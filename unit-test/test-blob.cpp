...
TEST(Blob, MallocBlobConcurrency)
{
    ...
    ASSERT_NO_THROW(proxy = skal::create_blob("malloc", "", size_B));
    ...
    std::thread thread(
            [size_B, &thread_ok, id] ()
            {
                try {
                    ...
                    proxy2 = skal::open_blob("malloc", id);
                    if (proxy2->id() != id) {
...
TEST(Blob, ShmBlobConcurrency)
{
    int64_t size_B = 1000;
    std::unique_ptr<skal::blob_proxy_t> proxy;
    ASSERT_NO_THROW(proxy = skal::create_blob("shm", "test-blob", size_B));
    ASSERT_NE(nullptr, proxy);
